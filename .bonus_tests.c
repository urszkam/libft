...
void test_ft_lstadd_front()
{
    ...
    // Test case 3: Add NULL node to a non-empty list
    t_list *list3 = ft_lstnew("Hello");
    t_list *old_head = list3;
    ft_lstadd_front(&list3, NULL);
    int result3a = list3 == old_head;
    int result3b = list3->next == NULL;
    printf("Test 3: ft_lstadd_front(\"Hello\", NULL) - First node: %s, Second node: %p - %s\n",
           (char *)list3->content,
           (void *)list3->next,
           result3a && result3b ? GREEN "PASS" COLOR_RESET
           : RED "FAIL" COLOR_RESET);
    ft_lstclear(&list3, &free_node);
}

void test_ft_lstadd_back()
{
    // Test case 3: Add NULL node to an empty list
    t_list *list1 = NULL;
    int content1 = 42;
    t_list *new_node1 = ft_lstnew(&content1);
    ...
    ft_lstadd_back(&list3, new_node3);
    int result3a = list3 == new_node3;
    printf("Test 3: ft_lstadd_back(NULL, NULL) - Result: %p - %s\n",
           (void *) list3, result3a ? GREEN "PASS" COLOR_RESET : RED "FAIL" COLOR_RESET);

}

void test_ft_lstlast()
{
    // Test case 1: List with one node
    t_list *list1 = NULL;
    int content1 = 42;
    t_list *new_node1 = ft_lstnew(&content1);
    ft_lstadd_back(&list1, new_node1);
    ...
    int result3a = last3 == NULL;
    printf("Test 3: ft_lstlast(empty list) - Last node: %s - %s\n",
           (char *)last3,
           result3a ? GREEN "PASS" COLOR_RESET : RED "FAIL" COLOR_RESET);
    ft_lstclear(&list3, &free_node);
}
...
void test_ft_lstmap()
{
    ...
    // Test case 2: Map an empty list
    t_list *list3 = ft_lstmap(NULL, &map_int, &free_node);
    int result2a = list3 == NULL;
    printf("Test 2: ft_lstmap(NULL) - Result: %p - %s\n",
           (void *)list3,
           result2a ? GREEN "PASS" COLOR_RESET : RED "FAIL" COLOR_RESET);
}

void test_ft_lstiter()
{
    // Test case 1: Iterate over a list with two int nodes
    t_list *list1 = NULL;
    int content1 = 1;
    int content2 = 2;
    t_list *new_node1 = ft_lstnew(&content1);
    ...
    ft_lstadd_back(&list1, new_node1);
    ft_lstadd_back(&list1, new_node2);
    ft_lstiter(list1, &double_int);
    int result1a = content1 == 2;
    int result1b = content2 == 4;
    printf("Test 1: ft_lstiter(list with two int nodes: 1,2) - Contents: %d, %d, Expected: 2, 4 - %s\n",
           content1, content2,
           result1a && result1b ? GREEN "PASS" COLOR_RESET
           : RED "FAIL" COLOR_RESET);
    ft_lstclear(&list1, &free_node);

    // Test case 2: Iterate over an empty list
    t_list *list2 = NULL;
    ft_lstiter(list2, &double_int);
    printf("Test 2: ft_lstiter(NULL) - Result: %p - %s\n",
           (void *)list2,
           list2 == NULL ? GREEN "PASS" COLOR_RESET : RED "FAIL" COLOR_RESET);
}
...
void test_ft_lstclear()
{
    ...
    // Test case 3: Delete a list with multiple nodes
    char *content4a = strdup("Hello");
    char *content4b = strdup("World");
    t_list *node4 = ft_lstnew(content4a);
    ft_lstadd_back(&node4, ft_lstnew(content4b));
    ft_lstclear(&node4, &free);
    printf("Test 3: ft_lstclear(list with two nodes) - Result: %p %s\n",
           (void *)node4,
           (node4 == NULL) ? GREEN "PASS" COLOR_RESET : RED "FAIL" COLOR_RESET);
}
